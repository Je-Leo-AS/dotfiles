...
# Script para desativar WARP e conectar ao Tailscale
# Autor: Script criado para alternar entre VPNs

set -e  # Para o script se houver erro

echo "🔄 Alternando de WARP para Tailscale..."
echo
...
if tailscale status >/dev/null 2>&1; then
    echo "ℹ️  Tailscale já estava conectado"
else
    echo "🔗 Conectando ao Tailscale..."
    if sudo tailscale up; then
        echo "✅ Tailscale conectado com sucesso"
    else
        echo "❌ Erro ao conectar ao Tailscale"
        exit 1
    fi
fi
